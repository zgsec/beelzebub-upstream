...
case "${1:-}" in ''|--bin) ;; *) echo "Usage: $0 [--bin]" >&2; exit 2 ;; esac
for command in docker git python3 curl jq; do
...
if [[ ${1:-} != --bin ]]; then
  command -v go >/dev/null || { echo 'Go is required to build.' >&2; exit 1; }
...
for side in stock fork; do
  ...
  [[ $version == *"$expected"* ]] || { echo "$side binary does not match pinned commit; rebuild without --bin." >&2; exit 1; }
done
